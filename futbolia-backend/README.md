...
│    ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  │
│    │ MongoDB  │  │ ChromaDB │  │ DeepSeek │  │ External │  │
│    │   (DB)   │  │  (RAG)   │  │  (LLM)   │  │   APIs   │  │
│    └──────────┘  └──────────┘  └──────────┘  └──────────┘  │
├─────────────────────────────────────────────────────────────┤
│                      DOMAIN LAYER                            │
...
### 3️⃣ Flujo de Predicción (Core Feature)

```
┌─────────────────────────────────────────────────────────────┐
│                   PREDICTION FLOW                            │
│            "El corazón de FutbolIA"                         │
└─────────────────────────────────────────────────────────────┘
...
└─────────────────────────────────────────────────────────────┘
              ...
              ▼                ▼                ▼
      ┌───────────────┐ ┌───────────────┐ ┌───────────────┐
      │  Auth Routes  │ │ Predict Routes│ │ Health Routes │
      │  /api/v1/auth │ │/api/v1/predict│ │   /health     │
      └───────┬───────┘ └───────┬───────┘ └───────────────┘
              │                 │
              ▼                 ▼
      ┌───────────────┐ ┌───────────────┐
      │  AuthUseCase  │ │PredictionUCase│
      └───────┬───────┘ └───────┬───────┘
              │                 │
              │     ┌───────────┴───────────┐
              │     │                       │
...
│   │   ├── chromadb/
│   │   │   ├── player_store.py    # Vector store
│   │   │   └── seed_data.py       # Datos iniciales
│   │   ├── llm/
│   │   │   └── dixie.py       # Integración DeepSeek
│   │   └── external_api/
│   │       └── football_api.py    # API externa
│   ├── use_cases/
│   │   ├── auth.py            # Lógica de autenticación
│   │   └── prediction.py      # Lógica de predicción
│   ├── presentation/
│   │   ├── auth_routes.py     # Rutas de auth
│   │   └── prediction_routes.py   # Rutas de predicción
│   └── main.py                # Entry point
├── data/
│   └── chromadb/              # Datos persistentes ChromaDB
...
4. **MongoDB Auth** - Usar autenticación en la base de datos
