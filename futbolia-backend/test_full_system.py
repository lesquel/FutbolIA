...
async def test_full_system():
    ...
    try:
        from src.infrastructure.external_api.api_football import APIFootballClient, ECUADORIAN_TEAMS
        
        print(f"Equipos ecuatorianos registrados: {len(ECUADORIAN_TEAMS)}")
        for name, info in list(ECUADORIAN_TEAMS.items())[:5]:
        ...
        if team_data:
            ...
            if squad:
                ...
                for p in squad[:5]:
                    print(f"   - {p['name']} ({p.get('position', 'N/A')}) #{p.get('number', 'N/A')}")
            else:
