...
async def test_international():
    ...
    for p in boca_raw[:5]:
        print(
            f"   - {p.get('name')} ({p.get('position')}) OVR:{p.get('overall_rating', p.get('overall', '?'))}"
        )
    ...
    result = await DixieAI.predict_match(
        boca, river, convert_players(boca_raw), convert_players(river_raw)
    )
    ...
    result2 = await DixieAI.predict_match(
        nacional, millos, convert_players(nacional_raw), convert_players(millos_raw)
    )
    ...
    result3 = await DixieAI.predict_match(
        flamengo, palmeiras, convert_players(flamengo_raw), convert_players(palmeiras_raw)
    )
    ...
    result4 = await DixieAI.predict_match(
        madrid, city, convert_players(madrid_raw), convert_players(city_raw)
    )
    ...
    result5 = await DixieAI.predict_match(
        america, chivas, convert_players(america_raw), convert_players(chivas_raw)
    )
    ...
    print("=" * 70)
    print(
        f"1. Argentina: Boca vs River -> {result.winner} ({result.predicted_score}) {result.confidence}%"
    )
    print(
        f"2. Ecuador: Nacional vs Millos -> {result2.winner} ({result2.predicted_score}) {result2.confidence}%"
    )
    print(
        f"3. Brasil: Flamengo vs Palmeiras -> {result3.winner} ({result3.predicted_score}) {result3.confidence}%"
    )
    print(
        f"4. Champions: Madrid vs City -> {result4.winner} ({result4.predicted_score}) {result4.confidence}%"
    )
    print(
        f"5. Mexico: America vs Chivas -> {result5.winner} ({result5.predicted_score}) {result5.confidence}%"
    )
    print("=" * 70)
