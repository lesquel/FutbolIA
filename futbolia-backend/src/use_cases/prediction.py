...
class PredictionUseCase:
    ...
    async def get_available_matches(cls) -> dict:
        ...
        def parse_match_date(date_str: str, time_str: str = None) -> Optional[datetime]:
                ...
                try:
                    async with httpx.AsyncClient(timeout=8.0) as client:
                        ...
                        if response.status_code == 200:
                            ...
                            print(f"📊 TheSportsDB returned {len(events)} events for {display_name}")
                            ...
                            print(f"✅ TheSportsDB: {events_checked} checked, {events_filtered_league} filtered (league), {events_filtered_past} filtered (past), {events_added} added")
                        else:
        ...
        for m in all_matches:
            ...
            if (match_date and 
                ...
                league in ["premier league", "english premier league"]):
        ...
        print(f"📊 Total matches after filtering: {len(future_matches)} (returning {len(all_matches)})")
        if future_matches:
        ...
        if not all_matches:
            print(f"⚠️ No matches from APIs (checked {len(seen_matches)} unique matches), using mock data with realistic dates (Premier League only)")
