...
class DatasetManager:
    ...
    def _load_json_sync(self, filepath: Path) -> Optional[Any]:
    ...
    async def get_standings(
        ...
        if data and "standings" in data:
            ...
            if isinstance(standings, list) and len(standings) > 0:
                # Puede venir como [{table: [...]}] o directamente como lista
                if isinstance(standings[0], dict) and "table" in standings[0]:
    ...
    async def get_teams(self, league_code: str) -> Optional[List[Dict]]:
        """
    ...
    async def get_matches(
        ...
        league_code: str,
        season: Optional[str] = None,
        status: Optional[str] = None,  # 'FINISHED', 'SCHEDULED', None=todos
    ...
    async def get_team_by_name(
        ...
        if league_code:
            ...
            for code in GLOBAL_LEAGUES.keys():
    ...
    async def get_team_squad(self, team_name: str) -> Optional[List[Dict]]:
    ...
    async def get_team_form(
        ...
        return {
            ...
            "losses": losses,
            "points": wins * 3 + draws,
            "form_rating": round((wins * 3 + draws) / (len(team_matches) * 3) * 100, 1) if team_matches else 0,
        }
    ...
    async def get_prediction_features(
        ...
        def extract_features(data: Dict, form: Optional[Dict], prefix: str) -> Dict:
    ...
    async def get_league_summary(self, league_code: str) -> Optional[Dict]:
