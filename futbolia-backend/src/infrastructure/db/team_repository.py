...
class TeamRepository:
    ...
    async def create(cls, team: Team, added_by: str = "system") -> Team:
        ...
        existing = await collection.find_one(
            {"$or": [{"api_id": team.id}, {"name": {"$regex": f"^{team.name}$", "$options": "i"}}]}
        )
    ...
    @classmethod
    async def find_by_name(cls, name: str) -> Team | None:
    ...
    @classmethod
    async def find_by_id(cls, team_id: str) -> Team | None:
        """Find team by ID"""
        collection = cls._get_collection()
    ...
    async def search(cls, query: str, limit: int = 20) -> list[Team]:
        ...
        cursor = collection.find({"name": {"$regex": query, "$options": "i"}}).limit(limit)
    ...
    async def update_player_status(cls, team_name: str, player_count: int) -> bool:
        ...
        result = await collection.update_one(
            ...
            {"$set": {"has_players": player_count > 0, "player_count": player_count}},
        )
