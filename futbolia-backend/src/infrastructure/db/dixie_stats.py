...
class DixieStats:
    ...
    @classmethod
    async def update_result(cls, prediction_id: str, actual_winner: str) -> bool:
        """
    ...
    async def get_overall_stats(cls) -> dict[str, Any]:
        ...
        high_conf_correct = await collection.count_documents(
            {"confidence": {"$gte": 0.7}, "is_correct": True}
        )
        high_conf_verified = await collection.count_documents(
            {"confidence": {"$gte": 0.7}, "actual_result": {"$ne": None}}
        )
        high_conf_accuracy = (
            (high_conf_correct / high_conf_verified * 100) if high_conf_verified > 0 else 0
        )
    ...
    async def get_team_stats(cls, team_name: str) -> dict[str, Any]:
        ...
        predicted_wins = await collection.count_documents(
            {**query, "predicted_winner": {"$regex": team_name, "$options": "i"}}
        )

        # Actual correct predictions for this team
        correct = await collection.count_documents({**query, "is_correct": True})

        verified = await collection.count_documents({**query, "actual_result": {"$ne": None}})
    ...
    async def get_recent_predictions(
        cls, limit: int = 10, user_id: str | None = None
    ) -> list[dict[str, Any]]:
