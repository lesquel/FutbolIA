...
from src.domain.entities import Match, MatchStatus, Prediction, PredictionResult, Team
...
class PredictionRepository:
    ...
    @classmethod
    async def find_by_id(cls, prediction_id: str) -> Prediction | None:
    ...
    def _doc_to_prediction(doc: dict) -> Prediction:
        ...
        if match_doc:
            home_team = (
                Team(**match_doc.get("home_team", {})) if match_doc.get("home_team") else None
            )
            away_team = (
                Team(**match_doc.get("away_team", {})) if match_doc.get("away_team") else None
            )
            ...
            match = Match(
                ...
                home_team=home_team,
                away_team=away_team,
                date=datetime.fromisoformat(match_doc.get("date", datetime.now(UTC).isoformat())),
