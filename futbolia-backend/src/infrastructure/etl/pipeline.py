...
class ETLPipeline:
    ...
    def __init__(self, config: PipelineConfig | None = None):
        self.config = config or PipelineConfig(football_data_key=settings.FOOTBALL_DATA_API_KEY)
    ...
    async def process_league(self, league_code: str, season: str | None = None) -> dict[str, Any]:
        """
        ...
        if self.config.include_teams and league.thesportsdb_id:
            teams_raw = await self.extractor.extract_league_teams_thesportsdb(league.thesportsdb_id)
            if teams_raw:
        ...
        # PASO 2: TRANSFORM
        # =====================================================================
        logger.info("🔄 TRANSFORM: Limpiando y normalizando datos...")
        ...
        # PASO 3: LOAD
        # =====================================================================
        logger.info("💾 LOAD: Guardando datos procesados...")
        ...
        if self.config.generate_ml_features and transformed_data["standings"]:
            ...
            if len(standings_data) >= 2:
                ...
                if ml_features:
                    await self.loader.load_ml_features(league_code, ml_features, "prediction")
    ...
    async def process_tier1_leagues(self) -> PipelineResult:
        ...
        return await self.process_leagues([lg.code for lg in tier1_leagues])
    ...
    async def process_leagues(
        ...
        if parallel:
            ...
            for code, res in zip(league_codes, results, strict=False):
    ...
    async def process_by_continent(self, continent: str) -> PipelineResult:
        ...
        return await self.process_leagues([lg.code for lg in leagues])
    ...
    async def quick_update(self, league_code: str) -> dict:
        ...
        standings = await self.extractor.extract_league_standings_thesportsdb(
            league.thesportsdb_id, int(league.get_current_season().split("-")[0])
        )

        if standings:
            transformed = DataTransformer.transform_standings(
                standings.get("raw_data", []), "thesportsdb"
            )
            await self.loader.load_standings(league_code, league.get_current_season(), transformed)
            result["updates"]["standings"] = True

        # Extraer próximos partidos
        next_matches = await self.extractor.extract_next_matches_thesportsdb(league.thesportsdb_id)

        if next_matches:
            transformed = DataTransformer.transform_matches(
                next_matches.get("raw_data", []), "thesportsdb"
            )
