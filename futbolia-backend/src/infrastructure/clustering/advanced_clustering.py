...
class ClusteringResult:
    ...
    inertia: float | None = None  # Solo para K-Means
...
class AdvancedClustering:
    ...
    def prepare_features(
        standings: list[dict[str, Any]], feature_set: str = "standard"
    ...
    def kmeans_clustering(
        ...
        for idx, (team_name, label) in enumerate(zip(team_names, labels, strict=False)):
    ...
    def dbscan_clustering(
        ...
        for idx, (team_name, label) in enumerate(zip(team_names, labels, strict=False)):
            ...
            team_data = {
                ...
                "is_outlier": label == -1,
                "features": {
                    name: round(float(features[idx][i]), 4) for i, name in enumerate(feature_names)
                },
                "position": standings[idx].get("position", idx + 1),
                "points": standings[idx].get("points", 0),
            }
    ...
    def find_optimal_clusters(
        standings: list[dict[str, Any]], max_clusters: int = 8, feature_set: str = "standard"
    ) -> dict[str, Any]:
        """
        ...
        for k in range(2, max_clusters + 1):
            ...
            results["elbow_method"].append({"k": k, "inertia": round(float(kmeans.inertia_), 4)})

            # Silhouette score
            if k < len(standings):
                silhouette = silhouette_score(features_scaled, labels)
                results["silhouette_method"].append(
                    {"k": k, "silhouette": round(float(silhouette), 4)}
                )
    ...
    def player_clustering(
        players: list[dict[str, Any]], n_clusters: int = 5, position_filter: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        if position_filter:
            players = [
                p for p in players if position_filter.lower() in p.get("position", "").lower()
            ]
        ...
        # Organizar resultados
        players_by_cluster = []
        for idx, (name, label) in enumerate(zip(player_names, labels, strict=False)):
    ...
    def _generate_cluster_descriptions(
        centroids: np.ndarray, feature_names: list[str], cluster_sizes: dict[int, int]
    ) -> list[dict[str, Any]]:
        ...
        cluster_order = sorted(range(len(centroids)), key=lambda i: centroids[i][0], reverse=True)
        ...
        for rank, cluster_idx in enumerate(cluster_order):
            ...
            feature_values = {
                name: round(float(centroid[i]), 3) for i, name in enumerate(feature_names)
            }
    ...
    @staticmethod
    def _interpret_dbscan_results(n_clusters: int, n_noise: int, total_teams: int) -> str:
    ...
    @staticmethod
    def _interpret_optimal_clusters(k: int, silhouette: float, n_teams: int) -> str:
