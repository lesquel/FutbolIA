...
class TeamClustering:
    ...
    def perform_clustering(
        ...
        cluster_info = TeamClustering._calculate_cluster_stats(
            teams_by_cluster, n_clusters
        )
            ...
            if len(unique_x) >= n_leaves:
                ...
                for pos, leaf_idx in enumerate(dendrogram_data["leaves"]):
                    if pos < len(unique_x):
                        ...
                        leaf_positions[leaf_idx] = min_x + (pos * (max_x - min_x) / (n_leaves - 1)) if n_leaves > 1 else min_x
    ...
    def _calculate_cluster_stats(
        teams_by_cluster: List[Dict[str, Any]],
        n_clusters: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        for cluster_id in range(1, n_clusters + 1):
            cluster_teams = [
                t for t in teams_by_cluster if t["cluster"] == cluster_id
            ]
