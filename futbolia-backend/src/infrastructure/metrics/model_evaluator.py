...
class EvaluationResult:
    ...
    brier_score: float
    log_loss: float
    roc_auc: Optional[Dict[str, float]]
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "cv_scores": {
                "mean": round(np.mean(self.cross_validation_scores), 4) if self.cross_validation_scores else None,
                "std": round(np.std(self.cross_validation_scores), 4) if self.cross_validation_scores else None,
            } if self.cross_validation_scores else None,
...
class ModelEvaluator:
    ...
    def _wilcoxon_mann_whitney_auc(probs: List[float], labels: List[int]) -> float:
        ...
        """
        positives = [p for p, l in zip(probs, labels) if l == 1]
        negatives = [p for p, l in zip(probs, labels) if l == 0]
    ...
    def temporal_analysis(
        ...
        """
        # Ordenar por fecha
        sorted_data = sorted(zip(dates, predictions, actuals), key=lambda x: x[0])
        
        temporal_metrics = []
        
        for i in range(window_size, len(sorted_data) + 1):
            window = sorted_data[i - window_size:i]
            window_dates, window_preds, window_actuals = zip(*window)
    ...
    def league_analysis(
        ...
        for pred, actual, league in zip(predictions, actuals, leagues):
