...
async def get_team_clustering(
    ...
    method: str = Query(
        default="ward", description="Linkage method (ward, complete, average, single)"
    ),
):
    """
    ...
    try:
        ...
        clustering_result = TeamClustering.perform_clustering(
            standings=standings, n_clusters=n_clusters, method=method
        )
        ...
        return {"success": False, "error": f"Error al realizar clustering: {str(e)}"}
...
async def get_custom_teams_clustering(
    ...
    try:
        ...
        for team in mongo_teams:
            ...
            sum(getattr(p, "passing", 70) for p in players) / len(players)
        ...
        log_info(
            "Custom teams clustering completed",
            n_clusters=n_clusters,
            n_teams=len(simulated_standings),
        )
