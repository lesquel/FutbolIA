...
import { Sparkles, BarChart3, Target, Star, Brain, Home, Bus, Trophy } from 'lucide-react-native';
import { useTheme } from '@/src/theme';
import { useTranslation } from '@/src/i18n/i18n';
import { ThemedText, Card, ConfidenceRing, TeamBadge, Button, Icon } from '@/src/components/ui';
...
const PlayerRow = memo(({ player, theme }: { player: Player; theme: any }) => (
  <View style={styles.playerRow}>
    <View
      style={[styles.positionBadge, { backgroundColor: getPositionColor(player.position, theme) }]}
    >
    ...
    <View style={[styles.ovrBadge, { backgroundColor: theme.colors.primary + '20' }]}>
...
const TeamSquadSection = memo(
  ({
    ...
    const avgOvr = useMemo(() => {
      ...
      return Math.round(players.reduce((a, p) => a + p.overall_rating, 0) / players.length);
    }, [players]);
    ...
    return (
      <View style={styles.squadSection}>
        ...
        <View style={[styles.squadBox, { backgroundColor: theme.colors.surfaceSecondary }]}>
          {players.length > 0 ? (
            players
              .slice(0, 11)
              .map((player, index) => <PlayerRow key={index} player={player} theme={theme} />)
          ) : (
...
export const PredictionCard = memo(function PredictionCard({
  ...
  const isDraw =
    result.winner.toLowerCase().includes('empate') || result.winner.toLowerCase().includes('draw');
    ...
    <Card variant="elevated" padding="lg">
      ...
      <View style={[styles.header, isLargeScreen && styles.headerLarge]}>
        <View style={styles.goalMindIcon}>
          <Icon icon={Sparkles} size={isLargeScreen ? 40 : 24} variant="primary" />
        </View>
      ...
      {/* Match Overview */}
      <View style={[styles.matchOverview, isLargeScreen && styles.matchOverviewLarge]}>
        <TeamBadge
        ...
        <View style={[styles.scoreContainer, isLargeScreen && styles.scoreContainerLarge]}>
          <ThemedText
      ...
      <View
        style={[
          ...
          {
            backgroundColor: isDraw ? theme.colors.warning + '20' : theme.colors.primary + '20',
      ...
      {/* Confidence Ring */}
      <View style={[styles.confidenceSection, isLargeScreen && styles.confidenceSectionLarge]}>
        <ConfidenceRing
      ...
      {/* Tactical Analysis */}
      <View style={[styles.analysisSection, isLargeScreen && styles.analysisSectionLarge]}>
        <View style={styles.sectionTitleRow}>
          <Icon icon={BarChart3} size={isLargeScreen ? 24 : 16} variant="primary" />
          <ThemedText
        ...
        >
          <ThemedText
            ...
            style={[styles.analysisText, isLargeScreen && styles.analysisTextLarge]}
          >
            {result.reasoning}
          </ThemedText>
        </View>
      ...
      {result.key_factors && result.key_factors.length > 0 && (
        <View style={styles.factorsSection}>
          <View style={styles.sectionTitleRow}>
            <Icon icon={Target} size={isLargeScreen ? 20 : 16} variant="primary" />
            <ThemedText
          ...
          {result.key_factors.map((factor, index) => (
            <View
              key={index}
              style={[styles.factorItem, { borderLeftColor: theme.colors.primary }]}
            >
      ...
      <View style={styles.starsSection}>
        <View style={styles.starPlayer}>
          ...
          <ThemedText weight="semibold">{result.star_player_home || 'N/A'}</ThemedText>
        </View>
        ...
        <View style={styles.starPlayer}>
          ...
          <ThemedText weight="semibold">{result.star_player_away || 'N/A'}</ThemedText>
        </View>
      </View>

      {/* Team Squads - Detailed Player Analytics */}
      {prediction.context && (
        <View style={[styles.squadsContainer, isLargeScreen && styles.squadsContainerLarge]}>
          <ThemedText
            size={isLargeScreen ? 'xl' : 'base'}
            weight="semibold"
            style={styles.sectionTitle}
          ...
          <View style={[styles.squadsGrid, isLargeScreen && styles.squadsGridLarge]}>
            <TeamSquadSection
      ...
      {result.tactical_insight && (
        <View style={styles.insightSection}>
          <View style={styles.sectionTitleRow}>
            <Icon icon={Brain} size={isLargeScreen ? 20 : 16} variant="primary" />
            <ThemedText
              size={isLargeScreen ? 'lg' : 'base'}
              weight="semibold"
              style={styles.sectionTitle}
          ...
          <View style={[styles.analysisBox, { backgroundColor: theme.colors.primary + '10' }]}>
            <ThemedText variant="secondary" size="sm">
              {result.tactical_insight}
            </ThemedText>
          </View>
